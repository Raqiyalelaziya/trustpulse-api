...
def get_shop(shop_id):
    ...
    cursor.execute(
        "SELECT r.*, u.full_name as reviewer_name, u.trust_score as reviewer_trust FROM reviews r LEFT JOIN users u ON r.user_id = u.id WHERE r.shop_id = %s AND r.is_approved = 1 ORDER BY r.created_at DESC",
        (shop_id,)
    )
...
@app.route("/trust/shop/<int:shop_id>", methods=["GET"])
...
def leaderboard():
    ...
    cursor = db.cursor(dictionary=True)
    cursor.execute("""
        SELECT u.id, u.full_name, u.trust_score, rp.points_balance, COUNT(r.id) as review_count
        FROM users u
